...
public final class SQLiteStationsRepository implements StationsRepository {
    ...
    public SQLiteStationsRepository(String dbUrl) {
        this.dbUrl = dbUrl;
        new StationsTableManager(dbUrl).ensureTableExists();
    }
    ...
    public void saveAll(List<Station> stations) {
        String sql = """
            INSERT OR REPLACE INTO stations (
                id, carrier_id, short_name, long_name, time_zone,
                latitude, longitude, is_meta_gare, address
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);
        """;
    ...
    public List<Station> findAll() {
        ...
        try (Connection conn = DriverManager.getConnection(dbUrl);
            ...
            System.err.println("❌ Error leyendo estaciones: " + e.getMessage());
        }
